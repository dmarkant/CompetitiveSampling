	...
	$.ajax({url: path,
			success: function(data) {
				$.each(data.split('\n'), function() {
					...
					if (optset[0]!="") {
						results.push({'id': Number(optset[0]),
                                      'set_ind': Number(optset[1]),
                                      'opt_ind': Number(optset[2]),
									  'A_common': Number(optset[3]),
									  'A_rare': Number(optset[4]),
									  'A_p': Number(optset[5]),
									  'A_ev': Number(optset[6]),
									  'B_common': Number(optset[7]),
									  'B_rare': Number(optset[8]),
									  'B_p': Number(optset[9]),
...
function sample_uniform_with_seed(x, n, seed) {
    var ran = new Random(seed);
    var i = Math.floor(ran.uniform(0, x));
    return i;
}

function sample_uniform_within_set_with_seed(n, set, seed) {
...
var Option = function(stage, id, n_options) {
    ...
    self.show_opponent_selection = function() {

        self.opp_selection = self.disp.append('g').attr('id', 'opp_selection');
        self.opp_selection.append('image')
                          .attr('x', self.x - 35)
                          .attr('y', self.y + 115)
                          .attr('width', 70)
                          .attr('height', 70)
